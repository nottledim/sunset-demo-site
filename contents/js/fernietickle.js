/*
  ...
  Last-modified: 2012-10-09  15:09:01 on penguin.lingbrae""
...
//	console.log(Highcharts.product + " version " + Highcharts.version);
	$.ajax({
	    url: PV.url + 'pvdata.php?pv=sum&xx=anon',
...
//    var check = $('#chtype2').attr('checked')?1:0;
    ...
    $.ajax({
	url: PV.url + 'pvdata.php?pv=day&date='+getdate+'&smooth='+check,
        success: function(daydata) {
	      ...
	      var now  = new Date();
////	      console.log("GetDate value: %s", getdate);
...
//		  console.log("future schedule time: %s incr: %s", sched, sched - now);
		  if (PV.timeout) clearTimeout(PV.timeout);
...
//    var width = Math.floor($('#resizer').width());
...
function chartResize() {
    var sz = set_img_size();
//    console.log("resize: w=%s h=%s", sz.w, sz.h);
...
//    $( "#chtype" ).buttonset();
    ...
    $('#weatherfcast').load('forecast.html .content');
...
/*
//   snippets for buttons no longer used
    $('#gohome').button({
        icons: {
